...
## 3. Seiza 0.10.0 baseline

PSF Guard pins the current published Rust integration versions. Seiza 0.10.0
...
Seiza 0.10.0's `SEIZAOB4` object catalog retains the memory-mapped spatial and
...
designations. In Seiza 0.10.0 it supports exact and prefix lookup, but not a
...
Seiza 0.10.0 uses one complete catalog bundle at `/data/v2/manifest.json`. It
...
`@seiza/astro-overlay` 0.5.0 is now the canonical browser overlay geometry and
...
Backend adapters must use the Seiza 0.10.0 owned-result and fallible-query APIs.
...
The paths are optional and resolved relative to `data_dir`. Seiza 0.10.0 owns
...
- Define shared API types with Seiza 0.10.0 stable object metadata.
