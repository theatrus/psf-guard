...
fn downgrade_to_v1(path: &std::path::Path) {
    Connection::open(path).unwrap().execute_batch("DROP TABLE project_intent_setup; DROP TABLE project_intent; DROP TABLE rig_setup; DROP TABLE site_snapshot; DROP TABLE site; PRAGMA user_version=1;").unwrap();
}
