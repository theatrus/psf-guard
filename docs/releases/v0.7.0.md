...
This release queues stack builds, resumes them as a night grows, and closes a
hole that let anyone on the network reach a server with no accounts.

Read the security note below before upgrading a server that listens on the
network. A server bound to anything other than loopback now refuses API
requests until an operator adds a user.
...
- A queue for stack builds. Build buttons stay live while a build runs, so
  ...
  no longer locks while mono channels build.
- A **Stop** button on a queued or running stack build, which becomes **Stop
  all** once a queue forms. The stop lands between frames, between channels,
  and between calibration masters. Nothing partial is published, and channels
  that already finished keep their previews.
- Rebuilds that only added frames resume from where the last build finished
  instead of integrating the night again, and the card counts the restored
  work. A checkpoint is reused only when it is provably an ancestor — same
  source fingerprints, calibration, policy, and pipeline — and a card that had
  to start over says which of those changed.
...
- Superseded stack artifacts are swept from the cache. Every rebuild wrote a
  new directory and the old ones accumulated without limit.
