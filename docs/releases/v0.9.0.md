...
PSF Guard now runs RC-Astro's BlurXTerminator, NoiseXTerminator, and
StarXTerminator on stack previews with star removal as its own stacked
output, fences calibration frames around optical changes, calibrates each
night of a multi-night stack with its own masters, and charts how every
stack's signal-to-noise grows with depth.
