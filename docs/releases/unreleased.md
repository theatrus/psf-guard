...
- A **Sky** view maps everything every database has pointed at on one
  ...
  the nights, from a start night you choose, and the stat band totals hours,
  frames, targets, nights, the sky covered once with overlaps counted once,
  the fields added together, the pixels the sky resolves into at the finest
  scale that reached each patch, and the longest night of any one rig. Each
  catalog's nights split at the quietest hour of its own day, so a rig
  imaging past noon UTC is not cut in two. Constellations and bright stars sit behind the fields; scroll to
