...
PSF Guard 0.9.6 exports each night's flats with its own lights for WBPP,
keeps stack previews current on their own, flags rotation skew and warm
sensors, and stops a drying dew spot in the flats from beading every stack.
...
- The TLS library is updated to rustls 0.23.45 for RUSTSEC-2026-0285
  (CVE-2025-61730), a TLS 1.3 handshake flaw in the versions before it.
  It reached the remote upload, sync, and update checks through their HTTPS
  clients.
