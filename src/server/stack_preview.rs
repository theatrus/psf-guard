...
/// the worker settles it into `cancelled` within a frame's work. Answering a
/// job that already finished is a 404 rather than a silent success, so the UI
/// can tell "stopped" from "too late".
...
fn run_group(
    ...
    let light_paths = group
        ...
        .collect::<Vec<_>>();
    // Building masters over a night of darks is minutes of work, so the stop
    // flag goes into Seiza's own per-frame check as well.
    let masters = crate::calibration::resolve_or_build_masters_for_group(
        &calibration_conn,
        cache_root,
        &light_paths,
        Some(&directory_tree),
        Some(seiza_stacking::CancelSignal::from(Arc::clone(cancel))),
    );
    if cancel.load(Ordering::Relaxed) {
