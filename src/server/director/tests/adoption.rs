...
#[tokio::test]
async fn blocked_catalog_commit_rolls_back_both_databases_and_allows_exact_retry() {
    let fixture = Fixture::new();
    fixture
        .source
        ...
        .unwrap();
    let preview = fixture.preview().await;
    // A rollback-journal reader permits BEGIN IMMEDIATE and writes, but blocks
    // COMMIT. This exercises the real catalog finalizer, not an injected error.
    fixture
        .source
    ...
    fixture.source.execute_batch("ROLLBACK;").unwrap();
    assert_eq!(
        crate::catalog_identity::read(&fixture.source).unwrap(),
        None
    );
    {
        ...
        let source_guid = Uuid::parse_str(
            fixture.plan["mappings"][0]["source_project_guid"]
                .as_str()
                .unwrap(),
        )
        .unwrap();
        assert_eq!(
            store.linked_project(fixture.catalog, source_guid).unwrap(),
            None
        );
        assert_eq!(
            store.linked_rig(fixture.catalog, "profile-a").unwrap(),
            None
        );
    }
    let (status, response) =
